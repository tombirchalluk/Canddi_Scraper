        ...
        for (var i = 0; i < words.length; i++) {
            ...
            if (regexFourNums.test(strCurrWord)) {  // if the current word matches the last four numbers regex.
                if (i > 0 && strCurrWord.length === 4) {    // if the last part of the number contains four digits.
                    ...
                    var strLastFourNums = knwl.tasks.removeCharacters(["(", ")"], words[i-1]);  // remove any brackets that could be in the number

                    if (regexThreeNums.test(strLastFourNums)) { // if the middle 3 numbers match the regex
                        strCurrWord = strLastFourNums + strCurrWord;  // add the last four numbers to these 3.
                    } 
